...
/* Schedule tuner — rewrites the poll updaters' cron windows from the houses'
   ...
   wall-clock time (plus FIXED_SLOTS, the daily ledgers that have no release
   habit), for the external clock (.build/dispatch-clock/): GitHub's
...
/* Slots the clock starts that no release habit sets: the two daily ledgers,
   whose missed day stays missing. prediction-refresh keeps one model record
   per Sydney date and np-score logs that day's bets, and each had only its
   one cron line, while GitHub dropped whole release windows on 14 and 21 Sep
   2026. The clock gives each a second, on-time trigger. Their cron lines
   stay, and a second run in a day is a no-op for both: the if-due gate, and
   a ledger that appends only what moved. */
export const FIXED_SLOTS = [
  { workflow: "np-score.yml", day: "daily", time: "07:15", label: "daily score (twin of its cron line)" },
  { workflow: "prediction-refresh.yml", day: "daily", time: "13:00", label: "daily model read (twin of its cron line)" },
];
export function dispatchTable(blocks) {
  ...
  for (const f of FIXED_SLOTS) slots.push({ ...f, _k: [f.day === "daily" ? -1 : DOW.indexOf(f.day), toMins(f.time)] });
