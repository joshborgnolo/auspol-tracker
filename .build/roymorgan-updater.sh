...
# ~/Library/LaunchAgents/local.auspol.roymorgan.plist from the copy in this
# directory). Every step logs one line to .build/logs/roymorgan.log; any
...
git add data/polls.json .build/roymorgan-src/ index.html feed.xml sitemap.xml robots.txt || { log "FAIL git add"; exit 1; }
