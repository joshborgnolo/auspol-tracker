#!/bin/bash
# Scheduled Spectre Strategy federal-poll update: extract -> if polls.json
# changed -> validate -> render-card -> build -> commit -> push. Installed
# via launchd (plist copied to ~/Library/LaunchAgents/local.auspol.spectre.plist
# from the copy in this directory). Every step logs one line to
# .build/logs/spectre.log; any failure exits non-zero before any commit,
...
EXTRACT_OUT="$(node .build/extract-spectre.mjs 2>&1)"
CODE=$?
...
case "$LAST_LINE" in
  SPECTRE_STATUS*) log "$LAST_LINE" ;;
  *) log "FAIL extract (no SPECTRE_STATUS line): $LAST_LINE"; exit 1 ;;
esac
...
log "new Spectre Strategy wave(s) detected; running validate/build/commit/push"
...
git add data/polls.json .build/spectre-src/ index.html feed.xml sitemap.xml robots.txt assets/auspol-card.png assets/auspol-card.json assets/auspol-latest.json assets/favicon.svg assets/favicon-192.png assets/favicon-192.json || { log "FAIL git add"; exit 1; }
MSG="Update Spectre Strategy poll data $(date '+%Y-%m-%d')"
if ! git commit -m "$MSG" >> "$LOG" 2>&1; then
  log "FAIL git commit"
  exit 1
fi
if ! push_main "$MSG" data/polls.json .build/spectre-src/ index.html feed.xml sitemap.xml robots.txt assets/auspol-card.png assets/auspol-card.json assets/auspol-latest.json assets/favicon.svg assets/favicon-192.png assets/favicon-192.json; then
  exit 1
fi
