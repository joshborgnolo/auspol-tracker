...
wave through the normal pipeline, and commit it directly on `main`, where
...
- STRAIGHT-TO-MAIN CONTRACT: you are checked out on `main` itself with no
  ...
  after your session ends. The updater wrappers' own push steps skip
  themselves (`AUSPOL_PR_GATE=1` is set for your session) — that skip is
