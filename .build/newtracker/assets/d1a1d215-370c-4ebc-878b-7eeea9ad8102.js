...
function CycleChart({ metric, cycles, mode, hidden, hi, setHi, lifted, unlift, chipClick, toggle,
  ...
  /* Six cards caption what the fan behind the singled-out line is made
     ...
     alone just move the start year), and – when the board has been cut
     to one outcome – "all terms", since the outcome clause itself is
     the selection (what remains on the board is exactly every term of
     that kind), and the static subtitle alone when the board is so thin
     there is no fan at all. The start year tracks the earliest term
     left on the board. */
  const cardSub = (() => {
    ...
    if (outcomeShown === "returned")
      return lead + ", with a historical fan chart for all terms since "
        + firstYear + " that ended in a returned government";
  ...
  return (
    <section className="card cycle-card">
      <div className="card-head cycle-head">
        <div>
          <h2 className="card-title">{M.key === "net" || M.key === "oppnet" ? (
            <>{M.title.replace("net approval", "")}
              <button type="button" className="hi-term"
...
/* URL state for the archive's two multi-selects (pollsters w=, tags h=) rides
   ...
   can take (every house name carries a space or "&", and no tag id is b
   followed by base36 digits), so old comma-joined links and mask links parse
   under one param. The orderings are FROZEN and append-only: a house or tag
   that arrives later takes a NEW high bit at the end, and bits naming things
   no longer in the archive are dropped, never trusted – an old link can
   never re-decode to different selections. Future houses APPEND to
...
const archMask = (order, set) => {
  let m = 0;
  order.forEach((name, i) => { if (set.has(name)) m += 2 ** i; });
  return "b" + m.toString(36);
};
/* Returns null when raw is not the mask form, so the caller falls back to
   the legacy comma-list path; bits beyond a frozen order decode to nothing. */
const archUnpack = (raw, order) => {
  if (!/^b[0-9a-z]+$/.test(raw || "")) return null;
  const v = parseInt(raw.slice(1), 36);
  const s = new Set();
  order.forEach((name, i) => { if (v % 2 ** (i + 1) >= 2 ** i) s.add(name); });
  return s;
};
...
function AllPollsView({ focus, onBack, backLabel, tppBasis, setTppBasis }) {
  ...
  /* The filtered table is a page in its own right, so its filters ride in
     ...
     one letter or two (the maps below); the two multi-selects (pollsters w=,
     tags h=) ride as b-prefixed base36 bitmasks over the frozen orderings
     declared beside POLL_TAGS, so a fully-dressed link stays short enough
     to survive a chat window. Restored here, once per mount
     ...
     values; links written before the bitmask scheme spelled the pollster
  ...
  const urlInit = (() => {
    ...
    return {
      ...
      has: (() => {
        const raw = get("h", "has") || "";
        const mask = archUnpack(raw, POLL_TAGS.map((t) => t.id));
          ...
          <thead>
            <tr>
              ...
              {facet === "primary" && pOrder.map((id) => {
                const c = PCOLS[id];
                return c.k
                  ? <ArchSortTh key={id} label={c.label} k={c.k} sort={sort} onSort={onSort} className={c.cls ? c.cls.trim() : undefined} />
...
function infoTerms(D) {
  ...
  /* A "Show the working" fold: the definition reads on its own, and the
