...
(() => {
  ...
  const compose = (panel) => {
    ...
    /* re-title inside the widen window too, so the one swapped row lands
       under the desktop ladder and is measured with everything else */
    const untitle = retitleFieldworkRow(panel, houseName(panel));
    try {
