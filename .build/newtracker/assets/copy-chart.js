...
(() => {
  ...
  /* a static image cannot be operated, so controls are stripped from the
     ...
     and axis labels. The Compare toggle's one home is under the hero legend
     (next to the lines it annotates) at every width, so it sits outside
     .hero-controls and must be stripped separately */
  const STRIP_SEL =
    "." + BTN_CLASS + ", .hero-controls, .hero-alt, .hero-chartbar, .hero > .pg-check";
  ...
  const readLegend = (target) =>
    [...target.querySelectorAll(".hl-item, .legend-chip")].map((i) => {
      ...
      return {
        ...
        /* a .legend-swatch chip drawn as a dashed rule (.legend-chip.dashed
           for a series dashed by basis, .residual for Others / Ind.) has a
           transparent swatch with a dashed border-top – read it as the
           dashed mark it is, not a square in whatever colour fell through */
        kind: /hl-line/.test(k) ? "line" : /hl-dashed/.test(k) ? "dashed"
            : /hl-band/.test(k) ? "band"
            : /legend-swatch/.test(k) ? (/\b(dashed|residual)\b/.test(i.className || "") ? "dashed" : "square")
            : "dot",
  ...
  const composeCardInner = (target, svgEl, own) => new Promise((resolve, reject) => {
      ...
      if (svgEl.querySelector(".cyc-band")) {
        ...
        if (years.length) {
          ...
          for (let i = 0; i < order.length; i++) {
            if (!member.has(order[i])) continue;
            const last = spans[spans.length - 1];
            if (last && member.has(order[i - 1])) last.push(order[i]);
            else spans.push([order[i]]);
          }
          const spanFmt = (run) => run.length === 1 ? String(run[0])
            : run[0] + "–" + (String(run[0]).slice(0, 2) === String(run[run.length - 1]).slice(0, 2)
    ...
    /* The Poll disagreement panel's chance-floor shading has no chip of its
       ...
       the past-terms band is named on the cycles chart. */
    if (target.classList.contains("ap-var"))
      legend.push({ label: "Chance floor (shaded)", kind: "shade", fill: inkVar("--ink-faint"), alpha: 1 });
        ...
        legLines.forEach((ln) => {
          ...
          ln.forEach((it) => {
            ...
            if (it.kind === "line" || it.kind === "dashed") {
              ...
              /* a shaded region: the area's own faint ink, at a weight a
                 swatch can be seen at (the chart draws it at 0.14) */
