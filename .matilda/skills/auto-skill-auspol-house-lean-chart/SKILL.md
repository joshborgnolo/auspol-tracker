...
Related: `auspol-extra-datapoint-pipeline` (per-poll datapoint passthrough) and `auspol-next-polls-projection` (another two-home gen-data↔renderer feature map). Sister panels: `auspol-poll-discord-panel` (VariancePanel directly above it — the pattern HouseLeanPanel is modelled on) and `auspol-flow-drift-panel` (mounted directly below it on the twopp facet — reuses this panel's chrome, `houseLeanColour`, hidden-chip toggle, bands className slot and the `--lean-*-bg` theme vars).
