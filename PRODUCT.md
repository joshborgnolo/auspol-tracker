...
- The page has four tabs. Snapshot: the headline, primary vote, the latest poll from each house, leader ratings and preferred PM, national direction, next expected polls, who votes for whom (by age, gender, education, state, location, housing and language at home), where One Nation's new voters came from, and the undecided share. Past cycles: the current term against the last twenty elections. All polls: the full poll list plus panels on poll disagreement, house lean and preference-flow drift. Info: the method, a reading guide and sources.
- Every page outside the main build shares the main site's masthead, tab bar, theme, live 2PP figure and colophon, from one module (`.build/site-shell.mjs`); the main tab bar links the archives. `/prediction/` and `/atlas/` carry that shell but are deliberately linked from nowhere.
